...
| 29/06/2026 | melhorias de cadastro, assinatura e alertas | tela de novo paciente reorganizada com e-mail de contato junto ao CPF, remoção do endereço comercial e destaque visual para Responsável Legal; criação/edição de Anamnese preserva os campos preenchidos quando a confirmação do clínico falha e reposiciona o usuário no bloco da assinatura; edição de paciente e ajuste de estoque preservam os dados digitados em falhas de senha/autorização; alertas `flash` do backend e validações JS passam a usar aviso global fixo e mais visível, sem depender do topo da página; rebuild do serviço web aprovado com `/health` local e HTTPS saudáveis; 41 testes focados aprovados |
