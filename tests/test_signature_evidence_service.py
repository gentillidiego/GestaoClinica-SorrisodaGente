...
def test_a_rogo_forms_require_only_clinical_user_credentials():
    ...
    for template in (atendimento_modal, tcle_form, anamnesis_form, anamnesis_edit):
...
def test_prepare_anamnesis_a_rogo_uses_clinical_credentials(monkeypatch):
    signer = {
        'id': 7,
        'username': 'dra.cibely',
        'role': Role.CLINICOS,
    ...
    signature_data = anamnesis_module._prepare_anamnesis_signature({
        'patient_not_literate': '1',
        'rogo_validator_username': 'dra.cibely',
        'rogo_validator_password': 'segura',
    })
    ...
    assert signature_data['declaration_text'] == signature_service.A_ROGO_DECLARATION
    assert signature_data['auth_method'] == 'login_senha_cd_a_rogo'
    assert signature_data['witnesses'] == []


def test_prepare_anamnesis_requires_patient_signature():
    with pytest.raises(ValueError, match='assinatura do paciente é obrigatória'):
...
def test_record_anamnesis_a_rogo_persists_evidence_fields(monkeypatch):
    ...
    assert params[5] == signature_service.A_ROGO_DECLARATION
    assert params[6] == '[]'
    assert params[7] == 'login_senha_cd_a_rogo'
